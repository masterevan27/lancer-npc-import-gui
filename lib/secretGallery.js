...
function createGallery({ config, configFile, paths, artStylesPath }) {
    const jobs = new Map();
    ...
    function isPrivate(file) { return [...privateRoots].some(dir => within(dir, file)); }
    ...
    function itemView(item) {
        ...
        return { id: item.id, name: item.name, kind: item.kind || 'npc', traits: item.traits || {}, callsign: item.callsign,
            when: item.when, seed: item.seed, artStyle: artStyles.metadata(item), secret: true, importable: false,
    ...
    function styleArgs(id) { const style = artStyles.select(artStylesPath, id || 'default', true); return ['--art-style', style.id, '--art-styles', artStylesPath, '--secret', '--secret-config', configFile]; }
    let dynamic = null;
    function scenes() {
        ready();
    ...
    return {
        ...
        setRoot(value) {
            ...
            root = next; privateRoots.add(next); dynamic = null; jobs.clear(); return root;
        },
        startCreate(kind, opts) {
            ...
            args.push(...styleArgs(opts.artStyle));
        ...
        renderCatalogue(body, catalogue) {
            ...
            args.push(...styleArgs(body.artStyle)); return start(args, 'background', false);
        },
    };
}
