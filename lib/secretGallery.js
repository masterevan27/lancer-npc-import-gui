...
const { dimensionArgs } = require('./imageDimensions');
...
function createGallery({ config, configFile, paths, artStylesPath, colorGuidancePath, workflowCatalog = require('./workflows').createCatalog(path.dirname(paths.generateNpcScript)), rerollableFor = () => [] }) {
    ...
    function itemView(item) {
        ...
        return { id: item.id, name: item.name, kind: item.kind || 'npc', traits: item.traits || {}, callsign: item.callsign,
            ...
            rerollable: [...rerollableFor({ ...item, kind: item.kind || 'npc' }), ...secretPrompts.secretSlots(item.secretPrompt)],
            // The template a Secret-mode NPC was prompted from, for the card and the detail sheet.
            secretPrompt: item.secretPrompt && typeof item.secretPrompt === 'object' && typeof item.secretPrompt.name === 'string'
                ? { file: item.secretPrompt.file, name: item.secretPrompt.name } : null,
            artStale: !!item.artStale, regenStatus: job?.status || null,
            hasRawTraits: !!(item.rawTraits && Object.keys(item.rawTraits).length),
    ...
    return {
        ...
        startCreate(kind, opts) {
            ...
            if (kind.id === 'npc') args.push('--tables', kind.tables);
            args.push(...dimensionArgs(opts));
        ...
        previewPrompts(kind, opts) {
            const args = kind.createArgs({ ...opts, count: 1, dryRun: true, server: null, promptLayout: undefined });
            args.push('--tables', kind.tables, '--prompt-preview', ...styleArgs(opts.artStyle, opts.workflow));
            if (colorGuidancePath) {
                const guidance = colorGuidance.select(colorGuidancePath, opts.colorGuidance || 'default', true);
                args.push('--color-guidance', guidance.id, '--color-guidance-catalog', colorGuidancePath);
            }
            return require('./promptComposer').preview(config.pythonExecutable, args);
        },
        startRegen(kind, item, body, trait = null, setTrait = null) {
            ...
            const newSeed = trait || setTrait || seedMode === 'random' ? crypto.randomInt(2 ** 32)
            ...
            if (setTrait) {
                args.push('--set-trait', `${setTrait.table}=${setTrait.value}`, '--apply-only');
                for (const name of setTrait.release) args.push('--release-trait', name);
            }
            ...
            if (kind.id === 'npc' && colorGuidancePath) {
                args.push('--color-guidance-catalog', colorGuidancePath);
                if (body.colorGuidance) {
                    const guidance = colorGuidance.select(colorGuidancePath, body.colorGuidance, true);
                    args.push('--color-guidance', guidance.id);
                }
            }
