...
// Each entry may carry a short, prompt-free `description` for the GUI to show
// beside its name; the built-in Default explains what the bare name hides.
const DEFAULT = Object.freeze({ id: 'default', name: 'Default', description: 'house painterly illustration', prompt: '', hidden: false });
...
function load(file) {
    ...
    const styles = data.styles.map(style => {
        ...
        if (style.description !== undefined && typeof style.description !== 'string') throw new Error('description must be a string');
        ids.add(style.id);
        return { id: style.id, name: style.name.trim(), description: (style.description || '').trim(), prompt: style.prompt.trim(), hidden: style.hidden === true || style.secret === true };
    });
...
function list(file, secret = false) { return load(file).filter(s => secret || !s.hidden).map(({ id, name, description }) => description ? { id, name, description } : { id, name }); }
