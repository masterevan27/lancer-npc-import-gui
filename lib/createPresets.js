...
function normaliseSettings(raw) {
    ...
    const pronouns = asString(raw.pronouns);
    ...
    const unarmed = asBoolean(raw.unarmed, false);
    ...
    return {
        ok: true,
        settings: { count, seed, pronouns, server, portrait, token, keepRawToken, unarmed, overrides },
    };
}
 ...
 */
function validatePresetFile(raw) {
    let data = raw;
    ...
    if (data.kind !== undefined && data.kind !== null && data.kind !== KIND) {
        return fail(`this file is a "${data.kind}" preset, not a Create NPC preset - Create NPC presets have "kind": "${KIND}"`);
    }
    if (data.settings === undefined && data.selected !== undefined) {
        return fail('this is a Tables preset (it lists "selected" tables), not a Create NPC preset (which carries "settings") - import it on the Tables tab instead');
    }
    if (data.settings === undefined) {
        return fail('this is not a Create NPC preset - it has no "settings"');
    }

    const name = typeof data.name === 'string' ? data.name.trim() : '';
    if (!name) return fail('preset name is missing');

    const result = normaliseSettings(data.settings);
    ...
    return { ok: true, preset: { name, created, kind: KIND, settings: result.settings } };
}
