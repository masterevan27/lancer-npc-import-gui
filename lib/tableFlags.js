...
const TABLE_FLAGS = {
    ...
    'Headgear': {
        ...
        mil: "Issued headgear - an officer's cap, a service helmet. Dropped for a civilian Role.",
        // Lock flags (admin, outlaw) are NOT listed here: they come from the
        // generator's ROLE_LOCKS through lib/gates.js, so the checkbox and
        // its gloss follow whatever the gate editor last wrote.
    },
    'Outfit': {
        ...
        lab: 'Laboratory wear - a lab coat. A preference, not a gate: Scientists are drawn toward it (ROLE_PREFERENCES), nobody is kept from it.',
    },
    'Faction': {
 ...
 */
function flagsFor(tableName, parents = {}, extra = {}) {
    ...
    //
    // `extra` is a second vocabulary keyed by base table, merged over the
    // literal one and never written into it: the gate flags the Tables tab
    // defines through lib/gates.js live in a sidecar file, not here, and a
    // lookup that mutated TABLE_FLAGS to include them would leak one tables
    // file's gates into every other kind's vocabulary for the server's life.
    // A gate flag that also has a literal gloss (admin, outlaw) takes the
    // extra's, which names who it admits NOW rather than who it admitted
    // when the gloss was written.
    const base = baseNameOf(tableName);
    const own = TABLE_FLAGS[tableName] || TABLE_FLAGS[base];
    const more = extra[tableName] || extra[base];
    if (own || more) return { ...(own || {}), ...(more || {}) };
    const parent = parents[tableName] || parents[base];
    return parent ? flagsFor(parent, {}, extra) : null;
}

/** Every flag name the GUI knows about for a table, variants included. */
function knownFlags(tableName, parents = {}, extra = {}) {
    return Object.keys(flagsFor(tableName, parents, extra) || {});
}

/** Whether this table has any flag vocabulary at all. */
function hasFlags(tableName, parents = {}, extra = {}) {
    return knownFlags(tableName, parents, extra).length > 0;
}
 ...
 */
function setBulletFlag(tableName, text, flag, on, parents = {}, extra = {}) {
    const isTheme = flag.startsWith('@');
    if (!isTheme && !knownFlags(tableName, parents, extra).includes(flag)) {
    ...
    const order = knownFlags(tableName, parents, extra);
