...
function createService({ script, tables, root, executable, jobs, onProduced = () => {}, timeoutMs = 1800000, previewTimeoutMs = 15000, extraArgs = () => [], urlPrefix = '/api/backgrounds/image', blockedFile = () => false, beforeStart = () => {} }) {
    ...
    function query(args, input) {
        return new Promise((resolve, reject) => {
            let child;
            try { child = childFor([...args, '--tables', tables, ...extraArgs(input?.artStyle, input?.workflow)]); } catch (err) { reject(err); return; }
    ...
    function start(args, input, kind, options = {}) {
        ...
        jobs.set(jobId, job);
        let child;
        try { child = childFor([...args, ...extraArgs(options.artStyle || input?.artStyle, options.workflow || input?.workflow)]); } catch (err) {
