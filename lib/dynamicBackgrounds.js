...
function createService({ script, tables, root, executable, jobs, onProduced = () => {}, timeoutMs = 1800000, previewTimeoutMs = 15000 }) {
    ...
    function query(args, input) {
        return new Promise((resolve, reject) => {
            let child;
            try { child = childFor([...args, '--tables', tables]); } catch (err) { reject(err); return; }
    ...
    function imageView(file) {
        ...
        return { rel, url: `/api/backgrounds/image?rel=${encodeURIComponent(rel)}&v=${fs.statSync(file).mtimeMs}` };
    }
    ...
    function start(args, input, kind, options = {}) {
        ...
        jobs.set(jobId, job);
        let child;
        try { child = childFor(args); } catch (err) {
    ...
    return {
        ...
        battlemap(body) {
            ...
            const result = start(['--battlemap', source, '--width', String(body.width ?? 1536), '--height', String(body.height ?? 1536),
                '--seed', String(body.seed ?? crypto.randomInt(2 ** 32)), '--notes', body.notes || '', '--output-dir', path.dirname(source)], undefined, 'battlemap');
