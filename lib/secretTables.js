...
function markdownTables(text) {
    ...
    for (const line of text.split(/\r?\n/)) {
        ...
        if (heading) {
            ...
            if (!name) throw new Error('a table name must be non-blank text');
...
function validateSelection(raw, listing, disableable, { checkGates = true } = {}) {
    ...
    // selection is refused here with the generator's own words. Skipped for
    // a preset that only loads, exports or imports an existing selection: a
    // fixed value can go stale when a table gains a `(when:)` after the
    // preset was saved, and the preset must still load so the form can show
    // the conflict rather than a bare 400.
    if (checkGates) {
