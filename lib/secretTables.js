...
/** '##' opens a table, '###' groups its choices, '- bullet' is a row. */
function markdownTables(text) {
    ...
    for (const line of text.split(/\r?\n/)) {
        ...
        if (heading) {
            current = heading[1];
            category = '';
            if (!counts.has(current)) counts.set(current, { values: [], groups: new Map() });
            continue;
        }
        ...
        if (current && /^-\s+\S/.test(line)) {
            ...
            const table = counts.get(current);
            table.values.push(value);
            if (!table.groups.has(category)) table.groups.set(category, new Set());
            table.groups.get(category).add(value);
        }
    }
    return [...counts].filter(([, table]) => table.values.length > 0)
        .map(([name, { values, groups }]) => ({ name, count: values.length, values: [...new Set(values)],
