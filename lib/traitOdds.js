...
/**
 * A cache key for one state of the tables file, for one kind.
 *
 ...
 *
 * `kind` is folded in because an NPC and a spaceship each have their own
 * tables file and their own odds run - without it, a cache keyed on the file
 * stat alone would still need a caller-side map to avoid one kind's odds
 * overwriting the other's, and the key would be lying about what it names.
 *
 ...
 */
function cacheKeyFor(stat, kind) {
