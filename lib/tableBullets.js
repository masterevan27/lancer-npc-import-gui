/**
 * Parses and edits npc-generator-tables.md's bullet lists in place.
 *
 * Mirrors generate-npc.py's own parse_tables()/split_flags() conventions
 * closely enough to stay in sync with it: '## Heading' starts a table, a
 * '- ' line is a bullet, a leading 'xN ' on a bullet is its weight. This
 * module adds one convention generate-npc.py doesn't need to know about -
 * a bullet wrapped '<!-- - text -->' is disabled. generate-npc.py's own
 * parser already ignores any line that doesn't start with '-', so a
 * disabled bullet is silently skipped there with no change needed to that
 * script - see the design doc this implements.
 */

const fs = require('node:fs');
const tableFlags = require('./tableFlags');

const HEADING_RE = /^##\s+(?!#)\s*(.*?)\s*$/;
const ENABLED_BULLET_RE = /^-\s+(.*?)\s*$/;
const DISABLED_BULLET_RE = /^<!--\s*-\s+(.*?)\s*-->\s*$/;
const WEIGHT_RE = /^x(\d+)\s+(.*)$/;

/**
 * Headings in npc-generator-tables.md that are documentation, not roll tables.
 *
 * The parser cannot tell them apart structurally: the '|| conventions' section
 * explains the format using '- ' bullets, which look exactly like roll
 * options. Served as options they got checkboxes and weight inputs, so
 * unchecking one wrapped a paragraph of the file's own documentation in
 * <!-- --> and setting a weight prefixed prose with 'x2 '.
 *
 * A named constant rather than a parsed rule, for the same reason the override
 * list is derived from REQUIRED_TABLES: the generator fixes these headings, so
 * they are not free to grow. A prose section added later with no bullets is
 * caught by the second half of isRollTable() without needing an entry here.
 */
const NON_TABLE_SECTIONS = [
    'How the script reads this file',
    'Prompt templates',
];

/** Whether a parsed '## Heading' section is something the GUI may edit. */
function isRollTable(name, bullets) {
    return !NON_TABLE_SECTIONS.includes(name) && bullets.length > 0;
}

/** One bullet line -> { raw, enabled }, or null if the line isn't a bullet at all. */
function matchBulletLine(line) {
    const disabled = line.match(DISABLED_BULLET_RE);
    if (disabled) return { raw: disabled[1], enabled: false };
    const active = line.match(ENABLED_BULLET_RE);
    if (active) return { raw: active[1], enabled: true };
    return null;
}

/** A bullet's raw text (weight prefix still attached) -> { weight, text }. */
function splitWeight(raw) {
    const m = raw.match(WEIGHT_RE);
    return m ? { weight: Number(m[1]), text: m[2] } : { weight: 1, text: raw };
}

/**
 * A group reference: a bullet whose prose is '=> Name' hands its slot to the
 * '## Name' table, whose bullets are the variants of one look. Mirrors
 * generate-npc.py's reference_target(): the prose is everything before the
 * table's flag segment, so a three-segment table is read with its own arity,
 * and a bare '=>' is not a reference - it rolls as text and gets noticed.
 */
const REFERENCE_PREFIX = '=> ';

function referenceTargetOf(tableName, text) {
    const prose = tableFlags.splitBulletFlags(tableName, text).body;
    const first = prose.split('||')[0].trim();
    if (!first.startsWith(REFERENCE_PREFIX)) return null;
    const target = first.slice(REFERENCE_PREFIX.length).trim();
    return target || null;
}

/** A heading's base table name: 'Hair (she) +' -> 'Hair'. (Same rule as lib/tableGroups.js.) */
function baseNameOf(name) {
    const paren = String(name).indexOf(' (');
    return paren === -1 ? String(name) : String(name).slice(0, paren);
}

/**
 * { groupHeading: parentBaseName } for every heading some table references,
 * and for that heading's own '(x)' and '(x) +' variants, which variant_table()
 * in the generator folds into the group. The parent is the referencing
 * table's BASE name: a reference in 'Outfit (she) +' still makes the group an
 * Outfit group, with Outfit's flags and Outfit's row in the Tables tab.
 */
function groupParents(tables) {
    const parents = {};
    const names = tables.map((t) => t.name);
    for (const table of tables) {
        for (const target of table.references || []) {
            for (const name of names) {
                if (name === target || name.startsWith(`${target} (`)) {
                    parents[name] = baseNameOf(table.name);
                }
            }
        }
    }
    return parents;
}

function parseTableFile(fileText) {
    const lines = fileText.split('\n');
    const tables = [];
    let current = null;
    for (const line of lines) {
        const heading = line.match(HEADING_RE);
        if (heading) {
            current = { name: heading[1], bullets: [], references: [] };
            tables.push(current);
            continue;
        }
        if (!current) continue;
        const bullet = matchBulletLine(line);
        if (!bullet) continue;
        const { weight, text } = splitWeight(bullet.raw);
        current.bullets.push({ text, weight, enabled: bullet.enabled });
        const target = referenceTargetOf(current.name, text);
        if (target && !current.references.includes(target)) current.references.push(target);
    }
    return tables;
}

function toggleBulletInText(fileText, tableName, bulletText, enabled) {
    if (NON_TABLE_SECTIONS.includes(tableName)) {
        return { ok: false, error: `"${tableName}" is not a roll table - it is the generator's own documentation` };
    }
    const lines = fileText.split('\n');
    let inTarget = false;
    for (let i = 0; i < lines.length; i++) {
        const heading = lines[i].match(HEADING_RE);
        if (heading) {
            inTarget = heading[1] === tableName;
            continue;
        }
        if (!inTarget) continue;
        const bullet = matchBulletLine(lines[i]);
        if (!bullet) continue;
        const { text } = splitWeight(bullet.raw);
        if (text !== bulletText) continue;

        if (bullet.enabled === enabled) return { ok: true, text: fileText }; // already in the requested state

        lines[i] = enabled ? `- ${bullet.raw}` : `<!-- - ${bullet.raw} -->`;
        return { ok: true, text: lines.join('\n') };
    }
    return { ok: false, error: `no bullet matching that text under "## ${tableName}"` };
}

/** The inverse of splitWeight(): weight 1 has no prefix, matching every plain bullet already in the file. */
function formatRaw(weight, text) {
    return weight === 1 ? text : `x${weight} ${text}`;
}

function setBulletWeightInText(fileText, tableName, bulletText, weight) {
    if (!Number.isInteger(weight) || weight < 1) {
        return { ok: false, error: 'weight must be an integer >= 1' };
    }
    if (NON_TABLE_SECTIONS.includes(tableName)) {
        return { ok: false, error: `"${tableName}" is not a roll table - it is the generator's own documentation` };
    }
    const lines = fileText.split('\n');
    let inTarget = false;
    for (let i = 0; i < lines.length; i++) {
        const heading = lines[i].match(HEADING_RE);
        if (heading) {
            inTarget = heading[1] === tableName;
            continue;
        }
        if (!inTarget) continue;
        const bullet = matchBulletLine(lines[i]);
        if (!bullet) continue;
        const { weight: currentWeight, text } = splitWeight(bullet.raw);
        if (text !== bulletText) continue;

        if (currentWeight === weight) return { ok: true, text: fileText }; // already at that weight

        const raw = formatRaw(weight, text);
        lines[i] = bullet.enabled ? `- ${raw}` : `<!-- - ${raw} -->`;
        return { ok: true, text: lines.join('\n') };
    }
    return { ok: false, error: `no bullet matching that text under "## ${tableName}"` };
}

/**
 * One bullet's `||` flag turned on or off, in place.
 *
 * Shaped like setBulletWeightInText() above and matching the same way - strip
 * the weight prefix, compare the remaining text verbatim, take the FIRST
 * match - but with one difference worth stating: this is the only writer here
 * that changes a bullet's TEXT, which is the key every other writer and every
 * saved preset matches on. lib/presets.js compensates by matching on the
 * flag-stripped body; see the comment on bulletKey() there.
 *
 * The flag vocabulary is per-table and lives in lib/tableFlags.js, which
 * rejects a flag the table does not read. That rejection is the point of the
 * whole feature: generate-npc.py matches flags literally and ignores an
 * unrecognized one rather than reporting it, so a typo fails quietly in the
 * render instead of loudly at the console.
 *
 * `parents` defaults to parsing `fileText` itself, which is right for every
 * caller that only has the text. setBulletFlagOnDisk() below passes its own
 * already-computed map instead, so the file is parsed for group parents once
 * per on-disk write rather than twice with two chances to disagree.
 */
function setBulletFlagInText(fileText, tableName, bulletText, flag, on, parents = groupParents(parseTableFile(fileText)), extraFlags = {}) {
    if (NON_TABLE_SECTIONS.includes(tableName)) {
        return { ok: false, error: `"${tableName}" is not a roll table - it is the generator's own documentation` };
    }
    if (typeof flag !== 'string' || !flag) {
        return { ok: false, error: 'flag must be a non-empty string' };
    }
    // A reference bullet ('=> Name') carries only @theme tags -
    // check_group_references() in generate-npc.py refuses any other flag on
    // one at startup ("flags belong on the group's members, the reference
    // takes only @theme tags"), so writing one here would hand back a file
    // the generator itself rejects before it ever reaches the tables it
    // describes. The Tables tab already hides the checkboxes for a reference
    // row (see renderBulletFlags() in app.js), but that is a rendering choice
    // a request that skips the client - replayed by hand, or a future caller -
    // would sail straight past, which is why the guard belongs here too.
    if (referenceTargetOf(tableName, bulletText) && !flag.startsWith('@')) {
        return {
            ok: false,
            error: `"${tableName}"'s "${bulletText}" is a group reference - it can carry only @theme flags, not "${flag}"`,
        };
    }
    const lines = fileText.split('\n');
    let inTarget = false;
    for (let i = 0; i < lines.length; i++) {
        const heading = lines[i].match(HEADING_RE);
        if (heading) {
            inTarget = heading[1] === tableName;
            continue;
        }
        if (!inTarget) continue;
        const bullet = matchBulletLine(lines[i]);
        if (!bullet) continue;
        const { weight, text } = splitWeight(bullet.raw);
        if (text !== bulletText) continue;

        const edited = tableFlags.setBulletFlag(tableName, text, flag, on, parents, extraFlags);
        if (!edited.ok) return edited;
        if (edited.text === text) return { ok: true, text: fileText }; // already in that state

        const raw = formatRaw(weight, edited.text);
        lines[i] = bullet.enabled ? `- ${raw}` : `<!-- - ${raw} -->`;
        return { ok: true, text: lines.join('\n') };
    }
    return { ok: false, error: `no bullet matching that text under "## ${tableName}"` };
}

/**
 * Many bullet edits applied in one pass over the file text.
 *
 * `edits` is a list of `{ table, text, enabled?, weight? }`. Omitting
 * `enabled` leaves the bullet's enabled state alone; omitting `weight` leaves
 * its weight alone; giving both rewrites the line once rather than twice.
 * Returns `{ text, applied, failed }`, where `failed` carries each rejected
 * edit with the reason - the caller decides what to do about it, which is the
 * whole point: applying a preset used to call the single-edit writers below
 * in a loop and discard every {ok:false} they returned, so a rejected write
 * was reported to the user as a success.
 *
 * Walking the file once also makes an apply O(file) rather than
 * O(edits x file): each single-edit writer re-reads, re-parses and re-writes
 * the whole of npc-generator-tables.md, so a 40-bullet preset was 40 full
 * rewrites, and the file was observably half-applied in between.
 *
 * Resolves a bullet to its FIRST matching line, the same rule
 * toggleBulletInText() and setBulletWeightInText() already follow. Where a
 * table carries the same bullet text twice, the later copies are unreachable
 * here as they always were - see docs/known-issues.md.
 */
function applyEditsInText(fileText, edits) {
    const lines = fileText.split('\n');

    // One indexing pass: table -> text -> [line index]. Built before any
    // rewriting so the line numbers stay valid; edits only ever replace a
    // line in place, never insert or delete one, so they cannot shift.
    const index = new Map();
    let current = null;
    for (let i = 0; i < lines.length; i++) {
        const heading = lines[i].match(HEADING_RE);
        if (heading) {
            current = heading[1];
            if (!index.has(current)) index.set(current, new Map());
            continue;
        }
        if (current === null) continue;
        const bullet = matchBulletLine(lines[i]);
        if (!bullet) continue;
        const { text } = splitWeight(bullet.raw);
        const byText = index.get(current);
        if (!byText.has(text)) byText.set(text, []);
        byText.get(text).push(i);
    }

    const applied = [];
    const failed = [];
    for (const edit of edits) {
        const { table, text } = edit;
        if (NON_TABLE_SECTIONS.includes(table)) {
            failed.push({ ...edit, error: `"${table}" is not a roll table - it is the generator's own documentation` });
            continue;
        }
        if (edit.weight !== undefined && (!Number.isInteger(edit.weight) || edit.weight < 1)) {
            failed.push({ ...edit, error: 'weight must be an integer >= 1' });
            continue;
        }
        const at = index.get(table)?.get(text);
        if (!at || !at.length) {
            failed.push({ ...edit, error: `no bullet matching that text under "## ${table}"` });
            continue;
        }
        // Re-read the line rather than trusting the index's snapshot of it:
        // an earlier edit in this same batch may already have rewritten it,
        // and a disable followed by a reweight should keep the disable.
        const i = at[0];
        const bullet = matchBulletLine(lines[i]);
        const { weight: currentWeight } = splitWeight(bullet.raw);
        const enabled = edit.enabled === undefined ? bullet.enabled : edit.enabled;
        const weight = edit.weight === undefined ? currentWeight : edit.weight;
        const raw = formatRaw(weight, text);
        lines[i] = enabled ? `- ${raw}` : `<!-- - ${raw} -->`;
        applied.push({ ...edit });
    }
    return { text: lines.join('\n'), applied, failed };
}

/**
 * A new, enabled bullet appended to one table - the GM's own custom value.
 *
 * Written directly after the table's last bullet line (enabled or disabled),
 * so it lands inside the table's list rather than after whatever blank line
 * or prose closes the section; a table with no bullets yet gets it under its
 * heading. Only a single line is ever inserted, so nothing else shifts.
 *
 * The text is refused rather than written whenever the file format would read
 * it back as something else:
 *   - more than one line, which would become a bullet and a stray line;
 *   - a leading 'xN ', which splitWeight() would take as the weight;
 *   - '<!--' or '-->', which would break the disabled-bullet wrapper the
 *     moment the new value is unchecked;
 *   - a '=> Name' group reference, which is structure the generator checks at
 *     startup and belongs in the file itself;
 *   - prose already present in the table. Every writer here and every saved
 *     preset resolves a bullet by that prose (flags aside - see bulletKey() in
 *     lib/presets.js), so a second copy would be unreachable from the tab.
 *
 * A '||' flag segment is kept verbatim and NOT checked against
 * lib/tableFlags.js: that vocabulary is keyed on heading name alone and
 * describes the NPC file, and the ship file's own '## Backdrop' reads flags
 * ('dock', 'combat') the NPC one does not, so gating on it would refuse real
 * ship values. The flag checkboxes remain the checked way to change them.
 */
function addBulletInText(fileText, tableName, bulletText, weight = 1) {
    if (NON_TABLE_SECTIONS.includes(tableName)) {
        return { ok: false, error: `"${tableName}" is not a roll table - it is the generator's own documentation` };
    }
    if (!Number.isInteger(weight) || weight < 1) {
        return { ok: false, error: 'weight must be an integer >= 1' };
    }
    if (typeof bulletText !== 'string') return { ok: false, error: 'text must be a string' };
    if (/[\r\n]/.test(bulletText)) return { ok: false, error: 'a value must be one line' };
    const text = bulletText.trim();
    if (!text) return { ok: false, error: 'a value cannot be empty' };
    if (WEIGHT_RE.test(text)) {
        return { ok: false, error: `"${text}" starts like a weight prefix - set the weight in the weight box instead` };
    }
    if (text.includes('<!--') || text.includes('-->')) {
        return { ok: false, error: 'a value cannot contain an HTML comment marker (<!-- or -->)' };
    }

    const tables = parseTableFile(fileText);
    const table = tables.find((t) => t.name === tableName);
    if (!table) return { ok: false, error: `no table "## ${tableName}"` };
    const parents = groupParents(tables);
    if (referenceTargetOf(tableName, text)) {
        return { ok: false, error: 'a group reference (=> Name) is structure - add it in the tables file itself' };
    }
    const body = tableFlags.splitBulletFlags(tableName, text, parents).body;
    const clash = table.bullets.find((b) => tableFlags.splitBulletFlags(tableName, b.text, parents).body === body);
    if (clash) {
        return { ok: false, error: `"## ${tableName}" already has that value${clash.enabled ? '' : ' (disabled)'}` };
    }

    const lines = fileText.split('\n');
    let headingAt = -1;
    let lastBulletAt = -1;
    for (let i = 0; i < lines.length; i++) {
        const heading = lines[i].match(HEADING_RE);
        if (heading) {
            if (headingAt !== -1) break; // left the target table
            if (heading[1] === tableName) headingAt = i;
            continue;
        }
        if (headingAt !== -1 && matchBulletLine(lines[i])) lastBulletAt = i;
    }

    // Match the file's own line endings, so a CRLF file stays one.
    const eol = lines[headingAt].endsWith('\r') ? '\r' : '';
    const line = `- ${formatRaw(weight, text)}${eol}`;
    if (lastBulletAt !== -1) {
        lines.splice(lastBulletAt + 1, 0, line);
    } else if (lines[headingAt + 1] !== undefined && lines[headingAt + 1].trim() === '') {
        lines.splice(headingAt + 2, 0, line, eol);
    } else {
        lines.splice(headingAt + 1, 0, eol, line);
    }
    return { ok: true, text: lines.join('\n'), bullet: { text, weight, enabled: true } };
}

function readTables(filePath) {
    return parseTableFile(fs.readFileSync(filePath, 'utf8'))
        .filter((table) => isRollTable(table.name, table.bullets));
}

function toggleBulletOnDisk(filePath, tableName, bulletText, enabled) {
    const fileText = fs.readFileSync(filePath, 'utf8');
    const result = toggleBulletInText(fileText, tableName, bulletText, enabled);
    if (!result.ok) return result;
    fs.writeFileSync(filePath, result.text);
    return { ok: true };
}

function setBulletWeightOnDisk(filePath, tableName, bulletText, weight) {
    const fileText = fs.readFileSync(filePath, 'utf8');
    const result = setBulletWeightInText(fileText, tableName, bulletText, weight);
    if (!result.ok) return result;
    fs.writeFileSync(filePath, result.text);
    return { ok: true };
}

/**
 * `extraFlags` is the gate vocabulary from the tables file's sidecar (see
 * lib/gates.js's gateFlagVocabulary), keyed by base table - without it a gate
 * flag the user defined a moment ago would be refused here as one the table
 * does not read.
 */
function setBulletFlagOnDisk(filePath, tableName, bulletText, flag, on, extraFlags = {}) {
    const fileText = fs.readFileSync(filePath, 'utf8');
    // Computed once and threaded into BOTH calls below, rather than each
    // parsing fileText for group parents on its own: a group table's write
    // and the text handed back to the caller must resolve against the same
    // map, or a group member's flag edit would succeed on disk while this
    // function's own re-derivation of the new text failed for want of a
    // parent to look up.
    const parents = groupParents(parseTableFile(fileText));
    const result = setBulletFlagInText(fileText, tableName, bulletText, flag, on, parents, extraFlags);
    if (!result.ok) return result;
    fs.writeFileSync(filePath, result.text);
    // The caller needs the new text back: the bullet's own text is its id, and
    // a flag edit has just changed it, so a client still holding the old one
    // would fail its next toggle or reweight against a bullet that no longer
    // exists under that name.
    return { ok: true, text: tableFlags.setBulletFlag(tableName, bulletText, flag, on, parents, extraFlags).text };
}

function addBulletOnDisk(filePath, tableName, bulletText, weight) {
    const fileText = fs.readFileSync(filePath, 'utf8');
    const result = addBulletInText(fileText, tableName, bulletText, weight);
    if (!result.ok) return result;
    fs.writeFileSync(filePath, result.text);
    return { ok: true, bullet: result.bullet };
}

/**
 * applyEditsInText() against a file, with a single read and a single write.
 *
 * The write is skipped entirely when nothing applied, so a batch that was
 * rejected in full leaves the file's mtime alone.
 */
function applyEditsOnDisk(filePath, edits) {
    const fileText = fs.readFileSync(filePath, 'utf8');
    const { text, applied, failed } = applyEditsInText(fileText, edits);
    if (applied.length) fs.writeFileSync(filePath, text);
    return { applied, failed };
}

module.exports = {
    parseTableFile, toggleBulletInText, readTables, toggleBulletOnDisk,
    setBulletWeightInText, setBulletWeightOnDisk, splitWeight,
    setBulletFlagInText, setBulletFlagOnDisk,
    addBulletInText, addBulletOnDisk,
    applyEditsInText, applyEditsOnDisk,
    isRollTable, NON_TABLE_SECTIONS,
    referenceTargetOf, groupParents, REFERENCE_PREFIX,
};
