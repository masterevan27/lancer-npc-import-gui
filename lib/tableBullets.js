 ...
 */
function setBulletFlagInText(fileText, tableName, bulletText, flag, on) {
    if (NON_TABLE_SECTIONS.includes(tableName)) {
        return { ok: false, error: `"${tableName}" is not a roll table - it is the generator's own documentation` };
    }
...
function setBulletFlagOnDisk(filePath, tableName, bulletText, flag, on) {
    const fileText = fs.readFileSync(filePath, 'utf8');
    const result = setBulletFlagInText(fileText, tableName, bulletText, flag, on);
    if (!result.ok) return result;
    fs.writeFileSync(filePath, result.text);
    ...
    return { ok: true, text: tableFlags.setBulletFlag(tableName, bulletText, flag, on).text };
}
