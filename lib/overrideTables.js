...
// The tables --disable-table accepts. Anchored like the others, and read off
// the generator rather than restated, for the reason REQUIRED_TABLES is: the
// list is the generator's to change, and a GUI checkbox for a table the
// script refuses would fail every run it was ticked on.
const DISABLEABLE_TABLES_RE = /^DISABLEABLE_TABLES\s*=\s*\(([\s\S]*?)\)/m;

/** Every table name in generate-npc.py's DISABLEABLE_TABLES, in file order. */
function disableableTablesFrom(pythonSource) {
    const block = pythonSource.match(DISABLEABLE_TABLES_RE);
    if (!block) return [];
    return [...block[1].matchAll(NAME_RE)].map((m) => m[1]);
}
