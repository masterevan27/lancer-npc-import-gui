 ...
 * weight }, where variantSubject is the pronoun subject of the option's own
 * heading, or null on a base table's bullets. Variants
...
function traitOptionsFrom(tables) {
    ...
    for (const table of tables) {
        ...
        // Read off the heading, not off isVariant: the two are not the same
        // question. A heading can be parenthesised - and so count as a variant
        // that folds into its base table - without naming a pronoun at all,
        // and those bullets must stay reachable by everyone.
        const variantSubject = variantSubjectOf(table.name);
        ...
        for (const bullet of table.bullets) {
            out[base].push({
                ...
                replacedFor: isVariant ? [] : (replacedFor[base] || []).slice(),
