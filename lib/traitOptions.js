...
/**
 * The headings that make up the group named `target`: the base table plus,
 * for each pronoun subject, whichever of `T (subject)` and `T (subject) +` is
 * actually present - mirroring generate-npc.py's group_headings() (see
 * generate-npc.py:1508-1531), generalised across every subject at once
 * instead of the one the caller asks for, since the picker has to enumerate
 * every option a group can produce rather than what one pronoun set can
 * reach.
 *
 * The rejected alternative is the one this replaces: fanning out with
 * `startsWith('${target} (')`, which was blind to WHAT the parenthesised text
 * named. 'Flight suits (gundam)' also starts with 'Flight suits (', but by
 * the tables file's own convention a themed sibling is a group of its own,
 * never a pronoun variant of the group it is styled after - it is reached
 * only through its own '=> Flight suits (gundam)' reference. Fanning out to
 * it here as well offered every one of its members twice: once mislabelled as
 * a member of `target`'s group, once correctly through its own reference.
 */
function groupHeadingsOf(byName, target) {
    const headings = [target];
    for (const subject of PRONOUN_SUBJECTS) {
        const replacement = `${target} (${subject})`;
        const additive = `${target} (${subject}) +`;
        // A present replacement wins outright - group_headings() never
        // returns both forms for one subject - and the additive form is
        // tried only when it isn't there, matching variant_table()'s own
        // "used INSTEAD OF" vs "added TO" distinction.
        if (byName.has(replacement)) headings.push(replacement);
        else if (byName.has(additive)) headings.push(additive);
    }
    return headings.filter((name) => byName.has(name));
}
...
function traitOptionsFrom(tables) {
    ...
    for (const table of tables) {
        ...
        const push = (bullet, heading, group, inheritedSubject = null) => {
            // Read off the heading, not off isVariant: the two are not the
            // same question. A heading can be parenthesised - and so count as
            // a variant that folds into its base table - without naming a
            // pronoun at all, and those bullets must stay reachable by
            // everyone. A group member with no pronoun of its own falls back
            // to the REFERENCING heading's subject: a group reached only from
            // '## Outfit (she) +' is a she-only group even when its own
            // heading, e.g. '## Crop tops', carries no parenthesis at all -
            // the generator's trait_choices offers such a group to she NPCs
            // only, and this mirrors it. A member's own subject still wins
            // when it has one, since that is more specific than where it was
            // reached from.
            const subject = variantSubjectOf(heading) ?? inheritedSubject;
            // Keyed off the OPTION's own base name, not the referencing
            // table's: a group's bare '(she)' replacement variant (no
            // trailing '+') gates the GROUP's own base bullets ('## Flight
            // suits'), not the table that references the group ('## Outfit').
            const memberBase = baseNameOf(heading);
            out[base].push({
                value: bullet.text,
                ...
                replacedFor: heading === memberBase ? (replacedFor[memberBase] || []).slice() : [],
        ...
        for (const bullet of table.bullets) {
            ...
            const referencingSubject = variantSubjectOf(table.name);
            for (const name of groupHeadingsOf(byName, target)) {
                const group = byName.get(name);
                if (!group) continue;
                for (const member of group.bullets) push(member, name, target, referencingSubject);
            }
        }
    }
