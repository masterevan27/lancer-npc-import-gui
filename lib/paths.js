...
function derivePaths(config) {
    ...
    // The user's own roll tables for Secret mode, read by generate-npc.py's
    // --extra-tables. Beside the tables file, where the generator repo already
    // gitignores it as prompts/secret-tables.
    const secretTablesDir = config.secretTablesDir
        || path.join(path.dirname(npcTablesPath), 'secret-tables');

    // The user's secret prompt templates, read by generate-npc.py's
    // --secret-prompts. A sibling of the secret tables folder rather than
    // of the tables file, so a GM who moved their tables folder finds the
    // templates beside it.
    const secretPromptsDir = config.secretPromptsDir
        || path.join(path.dirname(secretTablesDir), 'secret-prompts');
    ...
    const generateBackgroundScript = config.generateBackgroundScript
        || path.join(path.dirname(generateNpcScript), 'generate-background.py');
    const dynamicBackgroundTablesPath = config.dynamicBackgroundTablesPath
        || path.join(path.dirname(npcTablesPath), 'background-generator-tables.md');
    const backgroundPresetsDir = config.backgroundPresetsDir
        || path.join(presetsDir, 'backgrounds');

    return {
        generateBackgroundScript, dynamicBackgroundTablesPath, backgroundPresetsDir,
        ...
        stagedImportsDir, stagedRefsDir, presetsDir, createPresetsDir, secretTablesDir, secretPromptsDir,
