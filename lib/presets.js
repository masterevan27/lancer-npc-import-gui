 ...
 *
 * `parents` is groupParents() of the same tables, so a group referenced from
 * Backdrop is split in Backdrop's three-segment shape. Without it a member's
 * scene would be read as its flags, and every member sharing a shot phrase
 * ("A character portrait") would collapse into one key.
 */
function bulletKey(tableName, text, parents = {}) {
    return tableFlags.splitBulletFlags(tableName, text, parents).body;
}

function diffPresetAgainstTables(presetSelected, parsedTables) {
    const byTable = new Map(parsedTables.map((t) => [t.name, t.bullets]));
    const parents = groupParents(parsedTables);
    const willEnable = [];
    const willDisable = [];
    const willReweight = [];
    ...
    for (const [table, entries] of Object.entries(presetSelected || {})) {
        ...
        const selected = new Map(entries.map((e) => [bulletKey(table, e.text, parents), e.weight]));
        const liveKeys = new Set(bullets.map((b) => bulletKey(table, b.text, parents)));
        for (const { text } of entries) {
            if (!liveKeys.has(bulletKey(table, text, parents))) notFound.push({ table, text });
        }
        ...
        for (const bullet of bullets) {
            const key = bulletKey(table, bullet.text, parents);
