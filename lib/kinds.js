...
/**
 * Only the kinds whose generator script exists on disk. Drives
 * /api/categories: a GUI pointed at a generator repo without
 * generate-spaceship.py simply shows no Spaceships tab instead of a route
 * that 400s the moment someone clicks it.
 */
function available(kinds) {
    const result = {};
    for (const [id, entry] of Object.entries(kinds)) {
        if (fs.existsSync(entry.script)) result[id] = entry;
    }
    return result;
}
