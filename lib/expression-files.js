...
function migrateSidecarEntry(entry, sourcePortraitMtime, destinationPortraitMtime, destinationPortrait) {
    const migrated = { ...entry };
    const recordedMtime = Number(entry.source?.mtime);
    if (Number.isFinite(recordedMtime) && Number.isFinite(sourcePortraitMtime)
        && Math.abs(recordedMtime - sourcePortraitMtime) <= 0.01) {
        migrated.source = {
            ...entry.source,
            path: destinationPortrait,
            mtime: destinationPortraitMtime,
        };
    }
    return migrated;
}
