 ...
 *
 * `pingpong` is the one optional field, and only ever an opt-out: the script
 * ping-pongs by default and takes `--no-pingpong` to play the loop one way,
 * so the flag is emitted only when the panel switched it off. The same shape
 * lib/backgrounds.js's animateArgs has, for the same script.
 */
function animateArgs(scriptPath, { portrait, out, description, seed, pingpong = true } = {}) {
