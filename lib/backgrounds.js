...
/**
 * The names a still and its loop are copied into SillyTavern's backgrounds
 * folder under. That folder is flat and its picker shows the bare filename
 * as the label, so the rel's directory is dropped, _slug's dashes become
 * spaces and the whole thing goes lowercase - the shape of the backgrounds
 * that ship with SillyTavern ("landscape beach day.png") and of the loops the
 * GM had been copying over by hand before this existed.
 *
 * ComfyUI's _0000N_ counter is dropped when it is 1 and kept as a plain
 * " N" otherwise: two variants of one entry are two files on this side and
 * must stay two on that side, but "landing zone 1" for the only one there
 * is reads as a mistake. The loop's name follows the still's exactly as
 * animationFilesFor's does, so the pair sorts together in the picker.
 */
function sillyTavernNames(rel) {
    const s = String(rel || '');
    const base = s.slice(Math.max(s.lastIndexOf('/'), s.lastIndexOf('\\')) + 1);
    const dot = base.lastIndexOf('.');
    const ext = dot > 0 ? base.slice(dot).toLowerCase() : '';
    const stem = dot > 0 ? base.slice(0, dot) : base;
    const counter = /_(\d+)_$/.exec(stem);
    const words = (counter ? stem.slice(0, counter.index) : stem)
        .replace(/[-_]+/g, ' ').replace(/\s+/g, ' ').trim().toLowerCase() || stem.toLowerCase();
    const n = counter ? Number(counter[1]) : 1;
    const name = n > 1 ? `${words} ${n}` : words;
    return { still: `${name}${ext}`, loop: `${name} animated.webp` };
}

/**
 * The id a still wears on the Import tab. A background has no manifest entry
 * and so no minted id, but the grid, its checkboxes, Delete Selected and the
 * seen store are all keyed by id - so the rel path becomes one, behind a
 * prefix no manifest id can start with (those are `<kind>-<slug>-<seed>`).
 * The rel is kept verbatim rather than slugged, so relOf() is exact and the
 * id can be handed straight back to the routes that take a rel.
 */
const ID_PREFIX = 'bg:';

function idFor(rel) {
    return `${ID_PREFIX}${String(rel || '')}`;
}

/** The rel a background id names, or null for any id that is not one. */
function relOf(id) {
    if (typeof id !== 'string' || !id.startsWith(ID_PREFIX)) return null;
    const rel = id.slice(ID_PREFIX.length);
    return rel || null;
}
...
module.exports = {
    ...
    animationFilesFor, isAnimationFile, sillyTavernNames, ID_PREFIX, idFor, relOf, resolveInside,
    motionPromptsFrom,
