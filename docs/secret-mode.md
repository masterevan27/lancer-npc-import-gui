...
The generator owns `art-styles.json`, next to `generate-npc.py`. Set `artStylesPath` in GUI `config.json` to use a different catalog. Records have `id`, `name`, `prompt`, optional boolean `hidden` (or `secret`), and an optional string `description` that the list response carries and the selectors show in parentheses after the name (the built-in Default describes itself as the house painterly illustration). The reserved `default` and `none` styles are always present, even with missing/blank catalogs; malformed catalogs fail validation. Default preserves the original prompts. **None (no art style)** removes house rendering clauses and adds no `Art style:` suffix. Custom prompts may be empty or whitespace-only for the same effect; names must remain nonblank. Hidden records are only returned after Secret login. Prompt text is never part of the style-list response.

The colour guidance catalog (`color-guidance.json`, override with `colorGuidancePath`) follows the same rules under a `guidance` key and is served by `/api/color-guidance`. The reserved `default` and `none` entries are always present; **Default** lists itself as the house palette (greys, olive drab and rust), and **None (no colour guidance)** omits the palette sentence from the prompt. It applies to NPC creation and regeneration only; hidden entries likewise require Secret login.
