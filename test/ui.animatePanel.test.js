...
test('the panel carries a ping-pong checkbox, off by default, beside the seed modes', async (t) => {
    ...
    assert.match(panel[0], /<label><input type="checkbox" id="animate-pingpong"> Ping-pong the loop<\/label>/,
        'the checkbox must sit inside a label, and ship unchecked');

    // Every ping-pong box in the page - the NPC panel and Create Background's
    // chain, Animate and Dynamic traits boxes - plays one way unless ticked.
    const boxes = html.match(/<input type="checkbox" id="[\w-]*pingpong"[^>]*>/g) || [];
    assert.equal(boxes.length, 4, 'expected four ping-pong checkboxes');
    for (const box of boxes) assert.doesNotMatch(box, /\bchecked\b/, `${box} ships checked`);
});
...
test('the Animate button posts the checkbox, and the sheet follows the loop on disk', async (t) => {
    ...
    const open = /state\.animationOwnerId = null;[\s\S]{0,600}?el\.animatePingpong\.checked = false;/.exec(js);
